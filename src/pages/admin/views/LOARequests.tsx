...
export default function LOARequests({ updateNotifs }) {
    ...
    const approveRequest = (request) => {
        axiosInstance
            .put('/api/loa/' + request.id + '/')
            .then(res => {
                fetchRequests()
                enqueueSnackbar('Approved LOA for ' + request.user.first_name + ' ' + request.user.last_name, {
                    variant: 'success',
    ...
    const rejectRequest = (request) => {
        axiosInstance
            .delete('/api/loa/' + request.id + '/', { data: { reason: reason } })
            .then(res => {
                fetchRequests()
                enqueueSnackbar('Rejected LOA for ' + request.user.first_name + ' ' + request.user.last_name, {
                    variant: 'success',
                    autoHideDuration: 3000,
                    anchorOrigin: {
                        vertical: 'bottom',
    ...
    const RequestCard = ({ request }) => (
        <Col md={6}>
            <Card>
                <Card.Body>
                    <h4 className="text-black font-w700 mb-1">{request.user.first_name} {request.user.last_name}</h4>
                    <h6 className="text-gray font-w500 mb-3">{format(new Date(request.start), 'MMM dd, yyyy')} 🡢 {format(new Date(request.end), 'MMM dd, yyyy')}</h6>
                    <blockquote className="mb-4">
                        <em>{request.remarks}</em>
                    </blockquote>
                    <Button
                        variant="bg-green"
                        className="mr-2"
        ...
        <>
            <Fade bottom duration={1250} distance="50px">
                {requests.length > 0
                    ...
                    : <IconAlert variant="green" icon={RiCheckboxCircleLine} header="All caught up!">
                        <p className="m-0">There are currently no LOA requests pending review.</p>
                    </IconAlert>
                }
            </Fade>
            <Modal
            ...
            >
                <Modal.Header closeButton>
                    <Modal.Title>Confirm Approve LOA Request</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <p>Are you sure you would like to <b>approve</b> {currentRequest.user?.first_name} {currentRequest.user?.last_name}'s LOA request?</p>
                </Modal.Body>
                <Modal.Footer>
            ...
            >
                <Form>
                    <Modal.Header closeButton>
                        <Modal.Title>Confirm Reject LOA Request</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <p>Are you sure you would like to <b>reject</b> {currentRequest.user?.first_name} {currentRequest.user?.last_name}'s LOA request?</p>
